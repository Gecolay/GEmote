...
public class EmoteManager {
    ...
    public EmoteManager(GEmoteMain GPluginMain) { GPM = GPluginMain; }
    ...
    public void reloadEmotes() {
        ...
        if(GPM.getCManager().RESTORE) for(Player player : Bukkit.getOnlinePlayers()) restoreEmote(player);
    }
    ...
    public boolean startEmote(Player Player, GEmote Emote) {
        ...
        PrePlayerEmoteEvent preEvent = new PrePlayerEmoteEvent(Player, Emote);
        ...
        Bukkit.getPluginManager().callEvent(new PlayerEmoteEvent(Player, Emote));
    ...
    public boolean stopEmote(Player Player, boolean SaveRestore) {
        ...
        PrePlayerStopEmoteEvent preEvent = new PrePlayerStopEmoteEvent(Player, emote);
        ...
        if(GPM.getCManager().RESTORE && SaveRestore) GPM.getDManager().execute("INSERT INTO emote_save (uuid, emote) VALUES (?, ?)", Player.getUniqueId().toString(), emote.getId());

        Bukkit.getPluginManager().callEvent(new PlayerStopEmoteEvent(Player, emote));
